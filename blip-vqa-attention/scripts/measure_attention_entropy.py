...
PATCH_SIZE = 16
MASK_DIR = masks_root
output = Path("att_entropy/focused_entropy.jsonl")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply attention overrides and compute per-layer/head entropy for focus tokens."
    )
    parser.add_argument(
        "--layers",
        default="all",
    ...
    parser.add_argument(
        "--keep-cls",
        action="store_true",
        help="Keep the image CLS token when computing entropy (default drops it).",
    )
    parser.add_argument(
        "--save-plots",
        action="store_true",
    ...
    parser.add_argument(
        "--plots-dir",
        type=Path,
        default=Path("att_entropy/plots"),
...
def compute_entropy_tensor(
    ...
    *,
    ...
    _, _, _, num_keys = cross_attentions[0].shape
    key_start = 1 if drop_image_cls else 0
    num_effective_keys = num_keys - key_start
    if num_effective_keys <= 0:
        raise ValueError("Cannot compute entropy with zero effective key tokens.")

    max_entropy = math.log(num_effective_keys)
    layer_entropies: List[np.ndarray] = []

    for layer_tensor in cross_attentions:
        attn = layer_tensor[0]  # (heads, tokens, keys)
        attn = attn[:, :, key_start:]
        probs = attn / attn.sum(dim=-1, keepdim=True).clamp_min(1e-12)
        probs = probs.clamp_min(1e-12)
        entropy = -(probs * probs.log()).sum(dim=-1)  # (heads, tokens)
        layer_entropies.append(entropy.detach().cpu().numpy())

    entropies = np.stack(layer_entropies, axis=0)  # (layers, heads, tokens)
    return entropies, max_entropy


class MerStats(NamedTuple):
    mer: np.ndarray
    inside_rms: np.ndarray
    outside_rms: np.ndarray


def compute_mask_energy_ratio(
    cross_attentions: Sequence[torch.Tensor],
    *,
    drop_image_cls: bool = True,
    ...
    num_effective_keys = num_keys - key_start
    if num_effective_keys <= 0:
        raise ValueError("Cannot compute MER with zero effective key tokens.")
    ...
    inside_layers: List[np.ndarray] = []
    outside_layers: List[np.ndarray] = []
    ...
    for layer_tensor in cross_attentions:
        ...
        if mask_tensor is not None:
            inside_rms = torch.sqrt((probs_sq * mask_tensor).sum(dim=-1) / inside_den)
            outside_rms = torch.sqrt((probs_sq * outside_tensor).sum(dim=-1) / outside_den)
            mer = inside_rms / (inside_rms + outside_rms).clamp_min(1e-12)
        else:
            shape = probs_sq.shape[:-1]
            inside_rms = torch.zeros(shape, device=probs.device, dtype=probs.dtype)
            outside_rms = torch.zeros(shape, device=probs.device, dtype=probs.dtype)
            mer = torch.full(shape, 0.5, device=probs.device, dtype=probs.dtype)

        inside_layers.append(inside_rms.detach().cpu().numpy())
        outside_layers.append(outside_rms.detach().cpu().numpy())
        mer_layers.append(mer.detach().cpu().numpy())

    return MerStats(
        mer=np.stack(mer_layers, axis=0),
        inside_rms=np.stack(inside_layers, axis=0),
        outside_rms=np.stack(outside_layers, axis=0),
    )
...
def _print_record_summary(
    ...
    print(f"max entropy (nats): {max_entropy:.4f}")
    print(
        " idx | override | token                 | base  | new   | delta | base_norm | new_norm | delta_norm | base_MER | new_MER | delta_MER"
    )
    print(
        "-----+----------+-----------------------+-------+-------+-------+-----------+----------+------------+----------+---------+-----------"
    )
    for tok in record_tokens:
        ...
        print(
            ...
            f"{tok['baseline_mean_nats']:.4f} | {tok['override_mean_nats']:.4f} | {tok['delta_mean_nats']:+.4f} | "
...
def main() -> None:
    ...
    with output.open("w", encoding="utf-8") as writer:
        for idx, entry in enumerate(records, 1):
            ...
            try:
                baseline_entropies, max_entropy = compute_entropy_tensor(
                    baseline_cross_attentions,
                    drop_image_cls=not args.keep_cls,
                )
            except ValueError:
                if target_ids and is_target:
                    print(f"[warn] Skipping requested record '{rec_id or image_value}': baseline entropy failed.")
                skipped_fail += 1
                continue

            try:
                baseline_mer_stats = compute_mask_energy_ratio(
                    baseline_cross_attentions,
                    drop_image_cls=not args.keep_cls,
                ...
                if target_ids and is_target:
                    print(f"[warn] Skipping requested record '{rec_id or image_value}': baseline MER failed.")
                skipped_fail += 1
                continue
            ...
            try:
                override_entropies, _ = compute_entropy_tensor(
                    override_cross_attentions,
                    drop_image_cls=not args.keep_cls,
                )
            except ValueError:
                if target_ids and is_target:
                    print(f"[warn] Skipping requested record '{rec_id or image_value}': entropy computation failed.")
                skipped_fail += 1
                continue

            try:
                override_mer_stats = compute_mask_energy_ratio(
                    override_cross_attentions,
                    drop_image_cls=not args.keep_cls,
                    mask_weights=mask_vector,
                )
                ...
                if target_ids and is_target:
                    print(f"[warn] Skipping requested record '{rec_id or image_value}': MER computation failed.")
                skipped_fail += 1
                continue

            record_tokens: List[Dict[str, object]] = []
            num_tokens = min(len(tokens), baseline_entropies.shape[2], override_entropies.shape[2])
            norm_denom = max(max_entropy, 1e-12)
            ...
            for token_idx in range(num_tokens):
                if token_idx >= len(attention_flags) or attention_flags[token_idx] == 0:
                    continue
                baseline_matrix = baseline_entropies[:, :, token_idx]
                override_matrix = override_entropies[:, :, token_idx]
                ...
                baseline_mer = baseline_mer_stats.mer[:, :, token_idx]
                override_mer = override_mer_stats.mer[:, :, token_idx]
                ...
                token_entry: Dict[str, object] = {
                        ...
                        "baseline_nats_matrix": _format_matrix(baseline_matrix),
                        "baseline_norm_matrix": _format_matrix(baseline_norm),
                        "override_nats_matrix": _format_matrix(override_matrix),
                        "override_norm_matrix": _format_matrix(override_norm),
                        "delta_nats_matrix": _format_matrix(delta_matrix),
                        "delta_norm_matrix": _format_matrix(delta_norm),
                        ...
                        "baseline_mean_nats": baseline_mean,
                        "override_mean_nats": override_mean,
                        "delta_mean_nats": delta_mean,
                        "baseline_mean_norm": float(baseline_mean / norm_denom),
                        "override_mean_norm": float(override_mean / norm_denom),
                        "delta_mean_norm": float(delta_mean / norm_denom),
                        "baseline_mean_mer": float(baseline_mer.mean()),
                        "override_mean_mer": float(override_mer.mean()),
                        "delta_mean_mer": float(delta_mer.mean()),
                    }
                record_tokens.append(token_entry)
                running_entropy.append(override_mean)
            ...
            if target_ids and is_target:
                ...
                _print_record_summary(record_id_str, image_value, question, record_tokens, max_entropy)
    ...
    if running_entropy:
        mean_entropy = float(np.mean(running_entropy))
        std_entropy = float(np.std(running_entropy))
    else:
        mean_entropy = 0.0
        std_entropy = 0.0
    ...
    print(
        ...
        f"mean_entropy={mean_entropy:.4f} +/- {std_entropy:.4f}"
    )
    ...
    if target_ids:
        ...
        if missing_targets:
            ...
            print(f"[warn] No entropy results produced for requested id(s): {readable}")
