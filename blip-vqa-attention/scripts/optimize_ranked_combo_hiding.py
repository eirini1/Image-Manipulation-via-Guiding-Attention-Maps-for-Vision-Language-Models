...
def _build_cached_record(
    ...
    *,
    ...
    object_map = (gaussian_from_mask(mask_soft) * mask_soft).clamp_(0.0, 1.0)
...
def _optimize_hide_for_combo_answer(
    *,
    model,
    record: Dict[str, Any],
    pair_layer_heads: Dict[int, Tuple[int, ...]],
    ...
    raw_hide = torch.nn.Parameter(torch.tensor(0.0, device=device))
    opt = torch.optim.Adam([raw_hide], lr=float(lr))
    image = record["image"]
    question = str(record["question"])
    object_map = record["object_map"]

    best_loss = float("inf")
    best_hide = 0.5
    best_stats: Dict[str, float] = {
        "object_keep_ratio_weighted": 0.0,
        "object_hidden_ratio_weighted": 0.0,
        "global_hidden_ratio_mean": 0.0,
    }

    for _ in range(max(1, int(steps))):
        hide_strength = torch.sigmoid(raw_hide)
        hide_grid = _build_hide_grid(object_map, hide_strength, hide_floor)
        override_rows = {i: hide_grid for i in record["override_indices"]}
        ...
        try:
            applied_layers = _apply_pair_combo(
                ...
                originals=originals,
                alpha_like=torch.tensor(float(alpha_fixed), device=device),
            )
            weights = torch.tensor([1.0], device=device)
            loss = model(image, [question], answer=[answer], train=True, n=[1], weights=weights)
            total_loss = loss + float(naturalness_weight) * (hide_strength ** 2)
            opt.zero_grad()
            total_loss.backward()
            opt.step()
            ...
            if total_val < best_loss:
                ...
                best_stats = _summarize_hiding(object_map, hide_grid.detach())
        finally:
            if applied_layers:
                revert_override(applied_layers, originals)

    return best_hide, best_stats
...
def _plot_hide_histograms_for_record(
    *,
    record_id: str,
    ...
    for idx, summary in enumerate(answer_summaries):
        ...
        values = [float(v) for v in summary.get("hide_values", [])]
        if values:
            ax.hist(
                values,
                bins=max(1, int(bins)),
            ...
            ax.set_title(f"{answer}\nn=0", fontsize=9)
        ax.set_xlabel("Best hide_strength")
        ax.set_ylabel("Count")
    ...
    fig.suptitle(f"Hide-strength histograms per answer | record_id={record_id}", fontsize=12)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Per record: sample ranked pair-combos, load baseline answers from baseline_topk JSONL files, and "
            "optimize hide_strength for each (combo, baseline-answer)."
        )
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Input JSONL path(s) containing baseline_topk (repeat for multiple files).",
    )
    parser.add_argument(
        "--answers-glob",
        default=None,
        help="Glob pattern for baseline_topk JSONL files (supports recursive '**').",
    )
    parser.add_argument(
        "--baseline-topk-size",
        type=int,
    ...
    parser.add_argument("--steps-per-answer", type=int, default=30, help="Hide optimization steps for each pair.")
    parser.add_argument("--hide-lr", type=float, default=0.08, help="Hide optimizer learning rate.")
    parser.add_argument("--alpha", type=float, default=1.0, help="Fixed override alpha in [0,1].")
    parser.add_argument("--hide-floor", type=float, default=0.0, help="Minimum retained attention at full hide.")
    parser.add_argument("--naturalness-weight", type=float, default=0.05, help="Penalty on hide_strength^2.")
    parser.add_argument(
        "--out-record-jsonl",
        default="out/ranked_combo_hide_union_per_record.jsonl",
        help="Per-record hide distribution output JSONL.",
    )
    parser.add_argument(
        "--out-pair-jsonl",
        default="out/ranked_combo_hide_union_per_pair.jsonl",
        help="Per (record,combo,answer) best-hide output JSONL.",
    )
    parser.add_argument(
        "--out-hist",
        default="out/{record_id}/ranked_hide_histograms.pdf",
        help=(
            "Output path template for per-record hide histogram figure. "
    ...
    if args.steps_per_answer <= 0:
        parser.error("--steps-per-answer must be > 0.")
    if not (0.0 <= float(args.alpha) <= 1.0):
        parser.error("--alpha must be in [0,1].")
    if not (0.0 <= float(args.hide_floor) <= 1.0):
        parser.error("--hide-floor must be in [0,1].")
    if float(args.naturalness_weight) < 0.0:
        parser.error("--naturalness-weight must be >= 0.")
    ...
    for rec_idx, entry in enumerate(selected_records, 1):
        ...
        hide_values_by_answer: Dict[str, List[float]] = {k: [] for k in union_answer_order}
        object_hidden_values_by_answer: Dict[str, List[float]] = {k: [] for k in union_answer_order}
        global_hidden_values_by_answer: Dict[str, List[float]] = {k: [] for k in union_answer_order}
        ...
        for combo_idx, pair_map in enumerate(parsed_combos, 1):
            ...
            for answer_norm in union_answer_order:
                ...
                best_hide, hide_stats = _optimize_hide_for_combo_answer(
                    model=model,
                    record=cached,
                    pair_layer_heads=pair_map,
                    answer=answer_text,
                    steps=int(args.steps_per_answer),
                    lr=float(args.hide_lr),
                ...
                hide_values_by_answer[answer_norm].append(best_hide)
                object_hidden_values_by_answer[answer_norm].append(float(hide_stats["object_hidden_ratio_weighted"]))
                global_hidden_values_by_answer[answer_norm].append(float(hide_stats["global_hidden_ratio_mean"]))
                ...
                per_pair_rows.append(
                    {
                        ...
                        "object_hidden_ratio_weighted": float(hide_stats["object_hidden_ratio_weighted"]),
                        "global_hidden_ratio_mean": float(hide_stats["global_hidden_ratio_mean"]),
        ...
        for answer_norm in union_answer_order:
            hide_vals = np.asarray(hide_values_by_answer[answer_norm], dtype=np.float64)
            objh_vals = np.asarray(object_hidden_values_by_answer[answer_norm], dtype=np.float64)
            globh_vals = np.asarray(global_hidden_values_by_answer[answer_norm], dtype=np.float64)
            if hide_vals.size == 0:
                continue
            answer_summaries.append(
                {
                    ...
                    "answer_norm": answer_norm,
                    "hide_values": [float(v) for v in hide_vals.tolist()],
                    "hide_mean": float(np.mean(hide_vals)),
                    "hide_std": float(np.std(hide_vals)),
                    "hide_min": float(np.min(hide_vals)),
                    "hide_max": float(np.max(hide_vals)),
                    "hide_p25": float(np.quantile(hide_vals, 0.25)),
                    "hide_p50": float(np.quantile(hide_vals, 0.50)),
                    "hide_p75": float(np.quantile(hide_vals, 0.75)),
                    "object_hidden_values": [float(v) for v in objh_vals.tolist()],
                    "object_hidden_mean": float(np.mean(objh_vals)) if objh_vals.size > 0 else 0.0,
                    "global_hidden_values": [float(v) for v in globh_vals.tolist()],
                    "global_hidden_mean": float(np.mean(globh_vals)) if globh_vals.size > 0 else 0.0,
        ...
        answer_summaries_sorted = sorted(answer_summaries, key=lambda x: float(x["hide_mean"]), reverse=True)
        ...
        if str(args.out_hist).strip():
            ...
            _plot_hide_histograms_for_record(
                record_id=rec_id,
        ...
        per_record_rows.append(
            {
                ...
                "answers_source_type": "baseline_topk",
                "baseline_topk_size": int(args.baseline_topk_size),
                "alpha_fixed": float(args.alpha),
                "hide_floor": float(args.hide_floor),
                "naturalness_weight": float(args.naturalness_weight),
                "hide_hist_path": (str(hist_out_path) if hist_out_path is not None else None),
                "union_answer_hide_distributions": answer_summaries_sorted,
            }
        )
        print(
    ...
    if hist_written_paths:
        if len(hist_written_paths) == 1:
            print(f"[info] Wrote hide histogram figure: {hist_written_paths[0]}")
        else:
            print(f"[info] Wrote {len(hist_written_paths)} hide histogram figures.")
